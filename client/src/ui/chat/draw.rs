...
use crate::ui::chat::view::ChatState;

pub fn chat(f: &mut Frame, chat: &ChatState, authenticated: bool) {
...
fn draw_header(f: &mut Frame, area: Rect, chat: &ChatState, authenticated: bool) {
...
fn draw_middle(f: &mut Frame, area: Rect, chat: &ChatState) {
...
fn draw_chat_history(f: &mut Frame, area: Rect, chat: &ChatState) {
    ...
    let offset = max_offset.saturating_sub(0);
...
fn draw_users(f: &mut Frame, area: Rect, chat: &ChatState) {
...
fn draw_input(f: &mut Frame, area: Rect, _chat: &ChatState) {
    ...
    let x = area.x + 1 + UnicodeWidthStr::width(input) as u16;
    let y = area.y + 1;
    f.set_cursor(x, y);
}
